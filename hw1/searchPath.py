...
def UCS(width, height, start, target, config):
    ...
    while len(frontier)>0:
        flag=frontierCost.index(min(frontierCost))
        node=frontier.pop(flag)
        currentvalue=frontierCost.pop(flag)
        explored.append(node)
        if node[0]>0:
            # Go UP
            child = [node[0]-1,node[1]]
            ...
            if child not in frontier and child not in explored and config[child[0]][child[1]]!=-1:
                if child==target:
                    ...
                    break
                frontier.append(child)
                frontierCost.append(currentvalue+config[child[0]][child[1]])
                parent[childHash] = node
